...
function SaveGenderType({
  systemTypeObj,
  setShowSaveSystemType,
}: ISaveSystemProps) {
  const isEdit = systemTypeObj !== undefined;

  const [formData, setFormData] = useState<GenderTypeData>({
  ...
  const saveGenderType = async (e: React.FormEvent) => {
    e.preventDefault();

    if (isEdit) {
      await makeRequest(
        "PUT",
        globals.admin.genderType.edit,
        {
          id: systemTypeObj.id,
          name: formData.name,
        },
        ...
        undefined,
        "...מעדכן מין חיה",
        "מין החיה עודכן בהצלחה"
      );
    } else {
      await makeRequest(
        "POST",
        globals.admin.genderType.new,
        {
          name: formData.name,
        },
        true,
        ...
        undefined,
        "...יוצר מין חיה",
        "מין החיה נוצר בהצלחה"
      );
    }
  };

  return (
    <div className="SaveGenderType">
      <button
      ...
      <div className="save-entity-form-container">
        <h2 className="save-entity-form-title">
          {isEdit ? "עריכת סוג מין" : "הוספת סוג מין"}
        </h2>
        <form className="save-entity-form" onSubmit={(e) => saveGenderType(e)}>
          <FormInput
            labelText=":שם"
            name="name"
            placeholder="שם"
