...
export const getCaseDayPrimaryDataRow = (
  ...
  const rowOne = getCaseDayRowByIndex(caseDay, CASE_GRID_FIRST_DATA_COLUMN_INDEX);
  if (rowOne && rowOne.date && rowOne.date.trim() !== "") {
    return rowOne;
  }
  return getCaseDayRowByIndex(caseDay, CASE_GRID_TITLE_COLUMN_INDEX);
};
