...
const STORAGE_PATH_PREFIX = "photos/";
const DEFAULT_PATIENT_IMAGE = "/assets/images/default-patient-image.jpg";
...
export const resolvePatientImageSrc = (
  ...
  // Raw storage keys are not browser URLs; use fallback/default instead.
  ...
  if (trimmed.startsWith(API_PATH_PREFIX)) {
    return `${env.BASE_URL}${trimmed}`;
  }

  // Accept only root-relative URLs (e.g. /assets/...). Unknown relative paths are treated as invalid.
