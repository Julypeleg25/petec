...
function buildV2Docs(tables) {
  ...
  for (const [legacyTable, outName] of lookupMap) {
    ...
    out[outName] = rows.map((r) => {
      ...
      if (legacyTable === T.medicine) {
        ...
        if (r.measure_unit_id != null) doc.measureUnitId = idMap.get(T.measure_unit, r.measure_unit_id);
        ...
        if (r.total_dose != null) doc.totalDose = toNumber(r.total_dose) ?? r.total_dose;
        if (r.comments != null) doc.comments = r.comments;
        if (r.route_of_administration_id != null)
          doc.routeOfAdministrationId = idMap.get(T.roa, r.route_of_administration_id);
        if (r.dosage_frequency_id != null)
          doc.dosageFrequencyId = idMap.get(T.dosage_frequency, r.dosage_frequency_id);
        if (r.default_unit != null) doc.defaultUnit = r.default_unit;
      }
  ...
  // De-dupe animal_vitals by (animalTypeId, vitalsType). Keep the most recently updated row.
  if (Array.isArray(out.animal_vitals) && out.animal_vitals.length > 0) {
    const byKey = new Map();
    for (const row of out.animal_vitals) {
      const key = `${row.animalTypeId ?? ""}::${row.vitalsType ?? ""}`;
    ...
    out.animal_vitals = Array.from(byKey.values());
  }
  ...
  for (const r of getRows(T.case_daily_details)) {
    ...
    const row = {
      index: toNumber(r.index) ?? c.caseDetailsGrid.length,
      date: toDateOnlyString(r.date) ?? String(r.date ?? ""),
  ...
  for (const caseDoc of out.cases) {
    for (const row of caseDoc.caseDetailsGrid) {
      ...
      row.dateTime = Number.isFinite(dt.getTime()) ? dt : new Date(dateStr);
    }
