...
function buildV2Docs(tables) {
  ...
  const knownSchemas = ["petec", "vetmate"];

  const resolveLegacyTableName = (tableName, options = {}) => {
    const { required = false } = options;
    for (const schema of knownSchemas) {
      const candidate = `${schema}.${tableName}`;
      if (tables[candidate]) return candidate;
    }

    const suffix = `.${tableName}`;
    const fallbackKey = Object.keys(tables).find((key) => key.endsWith(suffix));
    if (fallbackKey) return fallbackKey;

    if (required) {
      throw new Error(`Could not find legacy table for ${tableName}. Available tables: ${Object.keys(tables).slice(0, 20).join(", ")}`);
    }
    return `petec.${tableName}`;
  };

  // Legacy tables (actual names from your dump)
  const T = {
    patient: resolveLegacyTableName("patient"),
    case: resolveLegacyTableName("case"),
    user: resolveLegacyTableName("user"),
    master_case: resolveLegacyTableName("master_case"),
    master_case_cases: resolveLegacyTableName("master_case_cases"),
    anesthesia: resolveLegacyTableName("anesthesia_procedure_form"),
    audit: resolveLegacyTableName("audit_log"),
    patient_document: resolveLegacyTableName("patient_document"),
    patient_document_type: resolveLegacyTableName("patient_document_type"),
    case_daily_details: resolveLegacyTableName("case_daily_details"),
    cdd_meds: resolveLegacyTableName("case_daily_details_medicines"),
    cdd_procs: resolveLegacyTableName("case_daily_details_procedures"),
    cdd_food: resolveLegacyTableName("case_daily_details_food_extras"),
    cdd_exams: resolveLegacyTableName("case_daily_details_examinations"),

    case_meds: resolveLegacyTableName("case_medicines"),
    case_procs: resolveLegacyTableName("case_procedures"),
    case_food: resolveLegacyTableName("case_food_extras"),
    case_exams: resolveLegacyTableName("case_examinations"),
    patient_medicine: resolveLegacyTableName("patient_medicine"),

    // lookups
    animal_type: resolveLegacyTableName("animal_type"),
    race_type: resolveLegacyTableName("race_type"),
    animal_color: resolveLegacyTableName("animal_color"),
    gender_type: resolveLegacyTableName("gender_type"),
    insurance_type: resolveLegacyTableName("insurance_type"),
    food_type: resolveLegacyTableName("food_type"),
    food_extra_type: resolveLegacyTableName("food_extra_type"),
    examination_type: resolveLegacyTableName("examination_type"),
    feces_type: resolveLegacyTableName("feces_type"),
    urine_type: resolveLegacyTableName("urine_type"),
    dosage_frequency: resolveLegacyTableName("dosage_frequency"),
    measure_unit: resolveLegacyTableName("measure_unit_types"),
    procedure_type: resolveLegacyTableName("procedure_type"),
    medicine: resolveLegacyTableName("medicine"),
    medicine_category: resolveLegacyTableName("medicine_category"),
    roa: resolveLegacyTableName("route_of_administration"),
    animal_vitals: resolveLegacyTableName("animal_vitals"),

    user_role: resolveLegacyTableName("user_role"),
    user_privilege: resolveLegacyTableName("user_privilege"),
    role_priv: resolveLegacyTableName("user_role_user_privilege"),
  };
...
if (legacyTable === T.medicine) {
  ...
  for (const rp of getRows(T.role_priv)) {
    const roleId = rp.user_role_id ?? rp.role_id;
    const privilegeId = rp.user_privilege_id ?? rp.privilege_id;
    if (roleId == null || privilegeId == null) continue;
    const rid = String(roleId);
    const pid = String(privilegeId);
...
function normalizeIdentityValue(value) {
  ...
  if (Array.isArray(value)) return JSON.stringify(value.map(normalizeIdentityValue));
  if (typeof value === "object") return JSON.stringify(value);
  return String(value);
}

function buildIdentityKey(doc, fields) {
  const parts = [];
  for (const field of fields) {
    const value = doc?.[field];
    if (value == null) return null;
    parts.push(`${field}:${normalizeIdentityValue(value)}`);
  }
  return parts.join("|");
}

function toMongoFilterValue(field, value, options = {}) {
  if (value == null) return value;
  const keepStringIdKeys = Array.isArray(options.keepStringIdKeys)
    ? options.keepStringIdKeys
    : [];
  if (
    ...
    !keepStringIdKeys.includes(field)
  ) {
...
async function findExistingDocsByIdentity(collection, docs, identityConfig) {
  const { fields, keepStringIdKeys = [] } = identityConfig;
  if (!fields || fields.length === 0 || docs.length === 0) return new Map();

  const firstField = fields[0];
  const firstFieldValues = [
    ...new Set(
      docs
        .map((doc) => doc?.[firstField])
        .filter((value) => value != null)
        .map((value) => toMongoFilterValue(firstField, value, { keepStringIdKeys })),
    ),
  ];

  if (firstFieldValues.length === 0) return new Map();

  const existingDocs = await collection
    .find({ [firstField]: { $in: firstFieldValues } })
    .project(Object.fromEntries(["_id", ...fields].map((field) => [field, 1])))
    .toArray();

  const existingByKey = new Map();
  for (const existing of existingDocs) {
    const key = buildIdentityKey(existing, fields);
...
async function insertAll(client, dbName, docsByCollection) {
  ...
  const identityConfigs = {
    animal_types: { fields: ["serialId"] },
    animal_colors: { fields: ["serialId"] },
    gender_types: { fields: ["serialId"] },
    insurance_types: { fields: ["serialId"] },
    food_types: { fields: ["serialId"] },
    food_extra_types: { fields: ["serialId"] },
    examination_types: { fields: ["serialId"] },
    feces_types: { fields: ["serialId"] },
    urine_types: { fields: ["serialId"] },
    dosage_frequencies: { fields: ["serialId"] },
    measure_unit_types: { fields: ["serialId"] },
    procedure_types: { fields: ["serialId"] },
    medicine_categories: { fields: ["serialId"] },
    medicines: { fields: ["serialId"] },
    routes_of_administration: { fields: ["serialId"] },
    race_types: { fields: ["serialId"] },
    anesthesia_form_texts: { fields: ["serialId"] },
    patient_document_types: { fields: ["serialId"] },
    animal_vitals: { fields: ["serialId"] },
    users: { fields: ["email"] },
    patients: { fields: ["serialId"] },
    master_cases: { fields: ["serialId"] },
    cases: { fields: ["serialId"] },
    anesthesia_forms: { fields: ["caseId"] },
    patient_documents: {
      fields: ["patientId", "caseId", "patientDocumentTypeId", "fileName", "storageKey", "uploadedAt"],
    },
    patient_medicines: {
      fields: [
        "patientId",
    ...
    audit_logs: {
      fields: ["entityType", "entityId", "subject", "description", "createdAt"],
      keepStringIdKeys: ["entityId"],
    },
  };
  ...
  for (const name of order) {
    const sourceDocs = docsByCollection[name];
    const docs = (sourceDocs || []).map((doc) => remapIdsDeep(doc, resolvedIdMap));
    if (!docs) continue;
    ...
    const identityConfig = identityConfigs[name];
    let existingByKey = new Map();
    if (identityConfig) {
      existingByKey = await findExistingDocsByIdentity(coll, docs, identityConfig);
    }

    const docsToInsert = [];
    let skippedExisting = 0;
    for (const doc of docs) {
      const key = identityConfig ? buildIdentityKey(doc, identityConfig.fields) : null;
      const existingDoc = key ? existingByKey.get(key) : null;
      if (existingDoc?._id) {
        resolvedIdMap.set(String(doc._id), String(existingDoc._id));
        skippedExisting += 1;
        continue;
      }
      resolvedIdMap.set(String(doc._id), String(doc._id));
      docsToInsert.push(doc);
    }

    const batchSize = 1000;
    const keepStringIdKeys =
      identityConfig?.keepStringIdKeys ?? [];
    for (let i = 0; i < docsToInsert.length; i += batchSize) {
      const batch = docsToInsert.slice(i, i + batchSize).map((d) =>
    ...
    console.log(`processed ${docs.length} docs in ${name} (${docsToInsert.length} inserted, ${skippedExisting} skipped existing)`);
  }
}
