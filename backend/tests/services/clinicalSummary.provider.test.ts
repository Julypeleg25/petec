...
const input = {
  ...
  pendingItems: [],
  sourceMetadata: {
    recordUpdatedAt: validResult.recordUpdatedThrough,
    inputWasTruncated: false,
  },
};
...
describe("Groq clinical summary provider", () => {
  ...
  it("uses the configured model, low temperature, no tools, and strict structured output", async () => {
    ...
    const { generateGroqClinicalSummary } =
      await import("../../src/services/clinicalSummary/clinicalSummary.provider.js");
    await expect(generateGroqClinicalSummary(input)).resolves.toEqual(
      validResult,
    );
  ...
  it("rejects invalid provider output", async () => {
      ...
      new Response(
        JSON.stringify({
          choices: [
            {
              message: {
                content: JSON.stringify({ backgroundAndAdmission: "partial" }),
              },
            },
          ],
        }),
    ...
    const { generateGroqClinicalSummary, ClinicalSummaryProviderError } =
      await import("../../src/services/clinicalSummary/clinicalSummary.provider.js");
    await expect(generateGroqClinicalSummary(input)).rejects.toEqual(
      expect.objectContaining<
        Partial<InstanceType<typeof ClinicalSummaryProviderError>>
      >({ category: "invalid_output" }),
    );
  });

  it("classifies malformed provider JSON as invalid output", async () => {
    jest
  ...
  it("maps provider timeouts to a safe category", async () => {
    jest.spyOn(globalThis, "fetch").mockRejectedValue(
      Object.assign(new Error("timed out with secret"), {
        name: "AbortError",
      }),
    );
    const { generateGroqClinicalSummary } =
  ...
  it("normalizes every visible date to day/month/year", async () => {
    jest.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(
        JSON.stringify({
          choices: [
            {
              message: {
                content: JSON.stringify({
                  ...validResult,
                  backgroundAndAdmission: "אושפז בתאריך 2026-01-31",
                  currentClinicalStatus: "נבדק שוב ב-31.01.2026",
                }),
              },
            },
          ],
    ...
    expect(result.backgroundAndAdmission).toContain("31/01/2026");
    expect(result.currentClinicalStatus).toContain("31/01/2026");
  });
});
